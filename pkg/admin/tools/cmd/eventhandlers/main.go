...
const ({{range .}}
  {{privateName .}}EventType = {{packetIndex .}}{{end}}
)
{{range .}}
...
func (eh {{privateName .}}EventHandler) Type() uint8 {
...
func init() { {{range .}}{{if isOpenttdEvent .}}
...
func main() {
	...
	names := []string{}
	for object, val := range parsedFile.Scope.Objects {
		if val.Kind == ast.Typ {
...
func isOpenttdEvent(name string) bool {
	switch {
...
func packetIndex(name string) string {
	if !isOpenttdEvent(name) {
		return "254 // internal handler"
	}

	return fmt.Sprint("packetIndexServer", name)
}
