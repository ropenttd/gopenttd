...
func TestDecodeInfoPacketNoGRF(t *testing.T) {
	...
	expectObject := util.OpenttdServerState{
		Status:        true,
		Error:         nil,
		Dedicated:     true,
	...
	output := util.OpenttdServerState{}
...
func TestDecodeCompanyPacket(t *testing.T) {
	...
	expectObject := util.OpenttdServerState{
		Companies: map[uint8]util.OpenttdCompany{
			0: {
				...
				Performance: 252,
				Passworded:  true,
				Vehicles: util.OpenttdTypeCounts{
					Train:    1,
					Truck:    0,
					Bus:      0,
					Aircraft: 2,
				...
				Stations: util.OpenttdTypeCounts{
					Train:    2,
					Truck:    0,
					Bus:      0,
					Aircraft: 2,
